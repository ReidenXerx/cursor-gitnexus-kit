...
describe('cursor-gitnexus-kit', () => {
  ...
  it('eval harness loads and validates task specs', async () => {
    ...
    const fxRoot = new URL(`../eval/fixtures/${checkable.fixture}/`, import.meta.url);
    assert.ok(fs.existsSync(new URL('verify.mjs', fxRoot)), 'fixture verify.mjs exists');
    assert.ok(fs.existsSync(new URL('../eval/runners/cursor-agent.mjs', import.meta.url)));
  });
