 ...
 * state under .cursor/. Those now live in the neutral .gnkit/ namespace, so remove
...
function cleanupLegacyHookLib(absTarget, actions) {
  ...
  if (fs.existsSync(legacyLib)) {
    try {
      ...
      actions.push("hooks: removed legacy .cursor/hooks/lib (moved to .gnkit/lib)");
    } catch {
      /* ignore */
    }
  }
  // Old skill-store location (moved to the tracked .gnkit/skills so teammates get
  ...
  if (fs.existsSync(legacyStore)) {
    try {
      ...
      actions.push("skills: removed legacy .gitnexus/agent-kit store (moved to .gnkit/skills)");
    } catch {
      /* ignore */
    }
  }
  for (const rel of [
    ...
    try {
      fs.unlinkSync(abs);
      actions.push(`hooks: removed legacy ${rel} (moved to .gnkit)`);
    } catch {
      /* ignore */
    }
  }
  // Per-session state flags/caches relocated from .cursor/ to .gnkit/.
