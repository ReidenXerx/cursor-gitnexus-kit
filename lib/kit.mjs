...
/** Seed region-profile doc on first install only (never overwrite project-specific profiles). */
function seedAgentProfilesIfMissing(targetRoot) {
  const dest = path.join(targetRoot, 'docs/AGENT-PROFILES.md');
  if (fs.existsSync(dest)) return;
  const stub = path.join(BUNDLE_ROOT, 'docs/AGENT-PROFILES.stub.md');
