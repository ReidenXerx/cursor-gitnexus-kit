...
const GITIGNORE_BASE = [
  ...
  // .gnkit/ holds the shared hook lib (tracked, like .cursor/hooks) + policy config;
  // only the per-session runtime state is ignored.
  ".gnkit/.gitnexus-*",
...
export function cliMain(argv) {
  ...
  if (!cmd || cmd === "--help" || cmd === "-h") {
    console.log(`Usage:
  node lib/kit.mjs install <target-repo> [--runtime cursor|zed|claude|both|all] [--repo-name NAME] [--quick] [--no-setup] [--skip-verify]
  node lib/kit.mjs install --interactive
  node lib/kit.mjs update <target-repo> [--runtime ...] [--full] [--no-setup] [--skip-verify]
  node lib/kit.mjs update-all <search-root> [--runtime ...] [--no-setup] [--skip-verify]
