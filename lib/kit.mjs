...
function seedFromStub(targetRoot, destRel, stubRel) {
  const dest = path.join(targetRoot, destRel);
  if (fs.existsSync(dest)) return false;
  const stub = path.join(BUNDLE_ROOT, stubRel);
...
/** Seed region docs on first install only (never overwrite project-specific files). */
function seedAgentProfilesIfMissing(targetRoot) {
  seedFromStub(targetRoot, 'docs/AGENT-PROFILES.md', 'docs/AGENT-PROFILES.stub.md');
}

function seedRegionsOverlayIfMissing(_targetRoot) {
  /* Intentionally not seeding regions.overlay.json — generate-regions uses skills/filesystem scan. */
}
...
export function installKit(targetRoot, opts = {}) {
  ...
  seedAgentProfilesIfMissing(absTarget);
  seedRegionsOverlayIfMissing(absTarget);
