...
export function generateArchDoc(root, repoArg, env) {
  ...
  if (!fs.existsSync(metaPath)) {
    return { written: false, path: ARCH_DOC_PATH, reason: 'no .gitnexus/meta.json — run gitnexus:refresh first' };
  }
  ...
  lines.push('');
  lines.push('> Auto-generated from the GitNexus knowledge graph. Do not edit by hand — regenerate with `npm run gitnexus:map`.');
  lines.push(`> Generated ${new Date().toISOString()}${lastCommit ? ` @ ${lastCommit.slice(0, 7)}` : ''}.`);
  lines.push('');
  ...
  if (clusters.length) {
    ...
    lines.push('');
    lines.push(`Explore an area: \`READ gitnexus://repo/${repo}/cluster/<Area>\``);
    lines.push('');
  }
  ...
  lines.push(`gitnexus_query({ query: "<concept>", repo: "${repo}" })   # orient (BM25 + embeddings)`);
  lines.push(`gitnexus_context({ name: "<symbol>", repo: "${repo}" })   # 360° on one symbol`);
  lines.push('```');
  lines.push('');
