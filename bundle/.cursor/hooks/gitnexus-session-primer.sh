...
node "$ROOT/.gnkit/lib/clear-session.mjs" "$ROOT" 2>/dev/null || true
...
STALENESS_JSON="$(node "$ROOT/.gnkit/lib/check-staleness.mjs" "$ROOT" 2>/dev/null || echo '{"fresh":false,"reason":"check_failed"}')"
...
if [[ "$IS_FRESH" != "true" ]] && [[ "${GITNEXUS_SKIP_SESSION_REFRESH:-}" != "1" ]]; then
  node "$ROOT/.gnkit/lib/set-refresh-pending.mjs" "$ROOT" set "$STALENESS_JSON" 2>/dev/null || true
  if node "$ROOT/scripts/gitnexus-agent.mjs" refresh; then
    ...
    node "$ROOT/.gnkit/lib/set-refresh-pending.mjs" "$ROOT" clear 2>/dev/null || true
  else
    node "$ROOT/.gnkit/lib/set-refresh-pending.mjs" "$ROOT" set-failed "session auto-refresh failed" 2>/dev/null || true
